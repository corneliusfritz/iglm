...
#' (via a `netplus` object), sampler controls, estimation controls, and storage
#' for results.
#' @references 
...
iglm_object_generator <- R6::R6Class("iglm_object",
                                       private = list(
                                         ...
                                         .calc_count_statistics = function() {
                                           counts <- as.vector(xyz_count_global(z_network =  private$.netplus$z_network,
                                                                                x_attribute =private$.netplus$x_attribute,
                                                                                y_attribute = private$.netplus$y_attribute,
                                                                                type_x = private$.netplus$type_x,
                                                                                type_y = private$.netplus$type_y,
                                                                                attr_x_scale = private$.netplus$scale_x,
                                                                                attr_y_scale = private$.netplus$scale_y,
                                                                                neighborhood = private$.netplus$neighborhood,
                                                                                overlap=private$.netplus$overlap,
                                                                                directed = private$.netplus$directed,
                                                                                terms = private$.preprocess$term_names,
                                                                                data_list = private$.preprocess$data_list,
                                                                                type_list = private$.preprocess$type_list,
                                                                                n_actor = private$.netplus$n_actor))
                                         ...
                                         .validate = function() {
                                           ...
                                           if(!is.null(private$.coef_popularity)){
                                             ...
                                             expected_length <- private$.netplus$n_actor +
                                               private$.netplus$directed*private$.netplus$n_actor
                                           ...
                                           if(!inherits(private$.netplus, "netplus")) {
                                             stop("Invalid netplus object in iglm_object.", call. = FALSE)
                                           }
                                       ...
                                       public = list(
                                         ...
                                         #'   name of a \code{\link{netplus}} object available in the calling environment. 
                                         ...
                                         initialize = function(formula = NULL, coef = NULL, 
                                           ...
                                           if(!is.null(file)){
                                             ...
                                             required_fields <- c("formula", "preprocess", "netplus", "coef", 
                                             ...
                                             private$.netplus <- netplus(x_attribute = data_loaded$netplus$x_attribute,
                                                                        y_attribute = data_loaded$netplus$y_attribute,
                                                                        z_network = data_loaded$netplus$z_network,
                                                                        n_actor = data_loaded$netplus$n_actor,
                                                                        type_x = data_loaded$netplus$type_x,
                                                                        type_y = data_loaded$netplus$type_y,
                                                                        scale_x = data_loaded$netplus$scale_x,
                                                                        scale_y = data_loaded$netplus$scale_y,
                                                                        neighborhood = data_loaded$netplus$neighborhood,
                                                                        directed = data_loaded$netplus$directed)
                                             ...
                                             private$.netplus <- private$.preprocess$data_object
                                         ...
                                         #'   The terms should correspond to methods available in the \code{\link{netplus}} 
                                         ...
                                         model_assessment = function(formula){
                                           ...
                                           names_tmp <- gsub("=", "_", names_tmp)
                                           names_tmp <- gsub(" ", "", names_tmp)
                                           observed <- eval_change(formula = formula,object = private$.netplus)
                                         ...
                                         plot = function(stats = FALSE, trace = FALSE, model_assessment = FALSE) {
                                           private$.results$plot(stats, trace, model_assessment)
                                         },
                                         #' @description
                                         ...
                                         #'   results, and the underlying \code{\link{netplus}} data object.
                                         gather = function(){
                                           list(formula = private$.formula,
                                                ...
                                                netplus = private$.netplus$gather())
                                         }, 
                                         #' @description
                                         ...
                                         estimate = function(){
                                           ...
                                           if(private$.control$estimate_model){
                                             if(private$.preprocess$includes_popularity){
                                               ...
                                               tmp <- private$.netplus$degree()
                                               if(private$.netplus$directed){
                                                 coefficients_popularity_internal[which(tmp$in_degree_seq ==0) + private$.netplus$n_actor] = NA
                                               ...
                                               if(private$.preprocess$includes_popularity){
                                                 private$.results$resize(size_coef = length(info$coefficients_nonpopularity), 
                                                                         size_coef_popularity = 
                                                                           (private$.netplus$n_actor + private$.netplus$n_actor* private$.netplus$directed)*
                                                                           private$.preprocess$includes_popularity)
                                             ...
                                             if(length(info$simulations) > 0){
                                               tmp = lapply(1:length(info$simulations),
                                                            function(x){netplus(x_attribute = info$simulations[[x]]$x_attribute,
                                                                                ...
                                                                                type_x = private$.netplus$type_x, 
                                                                                type_y = private$.netplus$type_y, 
                                                                                scale_x = private$.netplus$scale_x, 
                                                                                scale_y = private$.netplus$scale_y)})
                                               class(tmp) <- "netplus.list"
                                               attr(tmp, "neighborhood") <- netplus.neighborhood(private$.netplus$neighborhood)
                                         ...
                                         simulate = function (nsim = 1, only_stats = FALSE, display_progress=TRUE,
                                           ...
                                           private$.results$update(samples = info$samples, stats = info$stats)
                                           private$.validate()
                                         ...
                                         #' @return A list of \code{\link{netplus}} objects representing
                                         ...
                                         #' @param x A \code{\link{netplus}} `` object containing the new observed data.
                                         #' @return The \code{\link{iglm_object}} itself, invisibly.
                                         set_target = function(x) {
                                           if(!"netplus" %in% class(x)) {
                                             stop("The target object must be of class 'netplus'.", call. = FALSE)
                                           } else {
                                             private$.netplus <- x
                                           }
                                           if(private$.control$display_progress){
                                             cat("Target netplus object has been set successfully.\n")
                                           }
                                           private$.calc_count_statistics()
                                           private$.validate()
                                         }
                                       ...
                                       active = list(
                                         ...
                                         #' @field netplus (`netplus`) Read-only. The associated \code{\link{netplus}} R6 object containing the network and attribute data.
                                         netplus = function(value) { if(missing(value)) private$.netplus else stop("`netplus` is read-only.", call. = FALSE) },
                                         ...
                                         #' @field count_statistics (`numeric`) Read-only. A named vector of the observed network statistics corresponding to the model terms, calculated on the current `netplus` data.
...
#' encompasses the complete model specification, linking the formula, data (\code{\link{netplus}} object), 
...
#'
#' @param formula A model `formula` object. The left-hand side should be the
#'   name of a `netplus` object available in the calling environment. 
#'   See \code{\link{model_terms}} for details on specifying the right-hand side terms.
