...
#'   `n_actor` is determined from the `netplus` object in the formula. 
...
#'   for each simulation. The full simulated \code{netplus} objects are discarded to minimize
#'   memory usage. If \code{FALSE}, the complete simulated \code{netplus} objects are created
...
#'   \strong{not} part of the 'overlap' set defined in the `netplus` object, potentially
...
#'   at its initial state (from the \code{\link{netplus}} object) and only simulates the
...
#'     `sampler$n_simulation` where each element is a `netplus` object
#'     representing one simulated draw from the model. The list has the S3 class
#'     `"netplus.list"`. If `only_stats = TRUE`, this is typically an empty list.}
...
#'   \code{netplus} for the data object structure. 
#'
#' @export
...
simulate_iglm = function(formula,coef,coef_popularity = NULL, 
  ...
  tmp = lapply(1:length(res$simulation_networks),
               function(x){XYZ_to_R(x_attribute = res$simulation_attributes_x[[x]], 
                                    ...
                                    n_actor = n_actor, return_adj_mat = FALSE)})
  tmp = lapply(1:length(res$simulation_networks),
               function(x){netplus(x_attribute = tmp[[x]]$x_attribute,
  ...
  attr(tmp, "neighborhood") <- netplus.neighborhood(preprocessed$data_object$neighborhood)
