...
- `samples`:
  ...
  (\`list\` or \`NULL\`) Read-only. A list of simulated \`netplus\`
  objects (class \`netplus.list\`).
...
- `samples`:
  ...
  (list) A list of simulated \`netplus\` objects (class
  \`netplus.list\`). If provided, replaces any existing samples.
