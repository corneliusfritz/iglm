...
- coef_popularity:
  ...
  where \`n_actor\` is determined from the \`netplus\` object in the
  formula.
...
- only_stats:
  ...
  simulation. The full simulated `netplus` objects are discarded to
  minimize memory usage. If `FALSE`, the complete simulated `netplus`
...
- offset_nonoverlap:
  ...
  part of the 'overlap' set defined in the \`netplus\` object,
...
- fix_x:
  ...
  at its initial state (from the [`netplus`](netplus.md) object) and
...
- \`samples\`:
  ...
  class \`"netplus.list"\`. If \`only_stats = TRUE\`, this is typically
  an empty list.
...
sampler object, `netplus` for the data object structure.
