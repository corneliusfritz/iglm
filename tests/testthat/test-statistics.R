test_that('Test some sufficient statistics for directed networks (Poisson)', {
  ...
  xyz_obj_new <- iglm.data(neighborhood = neighborhood, directed = F, 
                           type_x = type_x, type_y = type_y)
  gt_coef = c(3, -1,-1)
  gt_coef_pop =  c(rnorm(n = n_actor, -2, 1))
  
  sampler_new = sampler.iglm(n_burn_in = 1, n_simulation = 5,
  ...
  sampler_new = sampler.iglm(n_burn_in = 1, n_simulation = 10,
                             sampler_x = sampler.net.attr(n_proposals =  n_actor*100,seed = 13),
                             sampler_y = sampler.net.attr(n_proposals =  n_actor*100, seed = 32),
                             sampler_z = sampler.net.attr(n_proposals = sum(neighborhood>0)*10, seed = 134),
                             init_empty = F)
  ...
  expect_all_true(as.vector(model_tmp_new$results$stats[,1] == count_statistics(model_tmp_new$results$samples ~ edges(mode = "local") )))
  expect_all_true(as.vector(model_tmp_new$results$stats[,2] == count_statistics(model_tmp_new$results$samples ~attribute_y )))
  expect_all_true(as.vector(model_tmp_new$results$stats[,3] == count_statistics(model_tmp_new$results$samples ~ attribute_x )))
  expect_all_true(as.vector(as.numeric(model_tmp_new$results$stats[,4]) - count_statistics(model_tmp_new$results$samples ~ spillover_xx_scaled_local) < 0.1))
  expect_all_true(as.vector(as.numeric(model_tmp_new$results$stats[,5]) - count_statistics(model_tmp_new$results$samples ~ spillover_xy_scaled_local) < 0.1))
  expect_all_true(as.vector(as.numeric(model_tmp_new$results$stats[,6]) - count_statistics(model_tmp_new$results$samples ~ spillover_yx_scaled_local) < 0.1))
  expect_all_true(as.vector(as.numeric(model_tmp_new$results$stats[,7]) - count_statistics(model_tmp_new$results$samples ~ spillover_yy_scaled_local) < 0.1))
  expect_all_true(as.vector(as.numeric(model_tmp_new$results$stats[,8]) - count_statistics(model_tmp_new$results$samples ~ spillover_xx_scaled_global) < 0.1))
  expect_all_true(as.vector(as.numeric(model_tmp_new$results$stats[,9]) - count_statistics(model_tmp_new$results$samples ~ spillover_xy_scaled_global) < 0.1))
  expect_all_true(as.vector(as.numeric(model_tmp_new$results$stats[,11]) - count_statistics(model_tmp_new$results$samples ~ spillover_yy_scaled_global) < 0.1))
  expect_all_true(as.vector(as.numeric(model_tmp_new$results$stats[,10]) - count_statistics(model_tmp_new$results$samples ~ spillover_yx_scaled_global) < 0.1))
...
test_that('Test some sufficient statistics for directed networks (Poisson)', {
  ...
  xyz_obj_new <- iglm.data(neighborhood = neighborhood, directed = TRUE, 
                           type_x = type_x, type_y = type_y)
  gt_coef = c(3, -1,-1)
  ...
  sampler_new = sampler.iglm(n_burn_in = 1, n_simulation = 100,
                             sampler_x = sampler.net.attr(n_proposals =  n_actor*100,seed = 13),
                             sampler_y = sampler.net.attr(n_proposals =  n_actor*100, seed = 32),
                             sampler_z = sampler.net.attr(n_proposals = sum(neighborhood>0)*10, seed = 134),
                             init_empty = F)
  ...
  model_tmp_new <- iglm(formula = xyz_obj_new ~ edges(mode = "local") + attribute_y + attribute_x + 
                          ...
                          spillover_yx_scaled_global +
                          spillover_yy_scaled_global + degrees,
                        coef = c(gt_coef,0,0,0,0,0),  coef_degrees = gt_coef_pop, sampler = sampler_new, 
                        control = control.iglm(accelerated = F,max_it = 200, display_progress = F))
  model_tmp_new$simulate()
  model_tmp_new <- iglm(formula = xyz_obj_new ~ edges(mode = "local") + attribute_y + attribute_x + 
                          spillover_xx_scaled_local + 
...
test_that('Test some sufficient statistics for undirected networks (Poisson)', {
  n_actor =100
  block <- matrix(nrow = 50, ncol = 50, data = 1)
  neighborhood <- as.matrix(Matrix::bdiag(replicate(n_actor/50, block, simplify=FALSE)))
  ...
  model_tmp_new <- iglm(formula = xyz_obj_new ~ edges(mode = "local") + attribute_y + attribute_x + 
                          ...
                          spillover_xy_scaled + 
                          spillover_yx_scaled +
                          spillover_yy_scaled + degrees,
                        coef = c(gt_coef,0,0,0,0),  coef_degrees = gt_coef_pop, sampler = sampler_new, 
                        control = control.iglm(accelerated = F,max_it = 200, display_progress = F))
  model_tmp_new$simulate(nsim = 4)
  expect_all_true(as.vector(model_tmp_new$results$stats[,1] == count_statistics(model_tmp_new$results$samples ~ edges(mode = "local") )))
  expect_all_true(as.vector(model_tmp_new$results$stats[,2] == count_statistics(model_tmp_new$results$samples ~attribute_y )))
  expect_all_true(as.vector(model_tmp_new$results$stats[,3] == count_statistics(model_tmp_new$results$samples ~ attribute_x )))
  expect_all_true(as.numeric(model_tmp_new$results$stats[,4]) == count_statistics(model_tmp_new$results$samples ~ spillover_xx_scaled))
  expect_all_true(as.numeric(model_tmp_new$results$stats[,5]) == count_statistics(model_tmp_new$results$samples ~ spillover_xy_scaled))
  expect_all_true(as.numeric(model_tmp_new$results$stats[,6]) == count_statistics(model_tmp_new$results$samples ~ spillover_yx_scaled))
  expect_all_true(as.numeric(model_tmp_new$results$stats[,7]) == count_statistics(model_tmp_new$results$samples ~ spillover_yy_scaled))
  as.numeric(model_tmp_new$results$stats[,7]) 
  count_statistics(model_tmp_new$results$samples ~ spillover_yy_scaled)
...
test_that('Test some sufficient statistics for undirected networks', {
  ...
  model_tmp_new <- iglm(formula = xyz_obj_new ~ edges(mode = "local") + attribute_y + attribute_x + spillover_yy_scaled + degrees,
                        coef = c(gt_coef,-1),  coef_degrees = gt_coef_pop, sampler = sampler_new, 
                        control = control.iglm(accelerated = F,max_it = 200, display_progress = F))
  model_tmp_new$simulate(nsim = 4)
  expect_all_true(as.vector(model_tmp_new$results$stats[,1] == count_statistics(model_tmp_new$results$samples ~ edges(mode = "local") )))
  expect_all_true(as.vector(model_tmp_new$results$stats[,2] == count_statistics(model_tmp_new$results$samples ~attribute_y )))
  expect_all_true(as.vector(model_tmp_new$results$stats[,3] == count_statistics(model_tmp_new$results$samples ~ attribute_x )))
  expect_all_true(as.numeric(model_tmp_new$results$stats[,4]) == count_statistics(model_tmp_new$results$samples ~ spillover_yy_scaled))
})
...
test_that('Test the spillover effects', {
  ...
  sampler_new = sampler.iglm(n_burn_in = 1, n_simulation = 3,
                             sampler_x = sampler.net.attr(n_proposals =  n_actor*10,seed = 13),
                             sampler_y = sampler.net.attr(n_proposals =  n_actor*10, seed = 32),
                             sampler_z = sampler.net.attr(n_proposals = sum(neighborhood>0)*10, seed = 134),
                             init_empty = T)
